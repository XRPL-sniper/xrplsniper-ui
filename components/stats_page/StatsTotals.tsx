...
export default function StatsTotals(
    ...
    const mintValues: Values[] = useMemo(() => {
        const mintValues: Values[] = []
        for (let i = 0; i < latestSummary?.length; i++) {
            mintValues.push({
                name: 'mint',
                time: latestSummary[i].date,
                value: latestSummary[i].mint
            })
        }
        return mintValues
    }, [latestSummary])
    console.log('mint values.')
    console.log(mintValues)
    ...
    //     console.log(`format values: ${values?.length}`)
    ...
    const formatValues2 = (values) => {
        console.log(`format values2: ${values?.length}`)

        const points: Points[] = []
        for (let i = 0; i < values?.length; i++) {
            points.push({time: values[i].time, value: values[i].value})
        }
    ...
    return (
        <>
            <div
                className="grid grid-flow-col grid-cols-1 grid-rows-2 gap-2 pb-8 sm:gap-4 md:grid-cols-2 md:grid-rows-1">
                <div
                ...
                <div
                    ...
                    style={{height: '350px'}}
                >
                    <TransactionsCountofTypeChart data={latestSummary} title={"Transactions By Type"}/>
                </div>
            </div>
            <div>
                <div
